...
- **데스크톱 사이드바**: `sticky top-0 h-[100dvh]` 뷰포트 높이 고정, 내비게이션 영역만 독립 스크롤(`overflow-y-auto`), 하단 유틸리티(마이페이지, 다크 모드, 오늘의 한 문장, 접기/펼치기) 뷰포트 하단 고정.
...
- **푸터 유형**: 공개 페이지용 리치 푸터(`public`), 앱/훈련 화면용 콤팩트 푸터(`app`), 실전 모의고사용 푸터 숨김(`none`).
