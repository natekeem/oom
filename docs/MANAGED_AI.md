...
Each feature has a finite, shared request/result schema. Unknown feature IDs and fields are rejected. Answer-feedback bounds remain question 2,000, context 600 and answer 8,000 characters. Script and roleplay inputs/results have their own bounded fields and list counts. Request bodies are capped before parsing, provider output is capped, all managed results are validated against the feature schema, and database result JSON is capped at 30KB. Script rewrite and roleplay generation now produce schema v2; parsers still accept stored v1 results. This is a result-contract extension only and requires no database migration because `ai_generation_results.result` already stores validated JSON with its schema version.
...
Prompt versions are `opic_answer_feedback_v1`, `opic_script_rewrite_v2`, and `opic_roleplay_question_v2`. Answer feedback retains schema v1; new script and roleplay generations use schema v2 while v1 remains renderable. Provider-reported input/output/cached token counts are recorded. Output and thought tokens are stored separately; thought tokens are added only for cost calculation. Absent token metadata remains null. Historical rows retain their old combined output count and have null thought_tokens; do not infer a historical split. No fabricated token counts. Pricing is a reservation-time catalog snapshot in micro-USD per million tokens:
...
STEP 4 script rewrite renders the original and rewritten script as an equal-width desktop comparison and a stacked mobile reading order. A deterministic browser-local word LCS marks meaningful changed words; whitespace, punctuation and capitalization-only differences stay quiet. Semantic change notes come from the validated result, remain short, and never trigger a second AI request. Stored v1 string notes are normalized only for display.

STEP 5 presents static and generated tasks through the same learner hierarchy: concise `상황`, visually primary `연습 질문`, and optional `핵심 체크`. New v2 roleplay results contain scenario, medium-length prompt and up to four cues. Old prompt-only results reuse the known static scenario and do not invent cues. Generated tasks are explicitly practice material, never official or past OPIc questions. Generation failure leaves the current static question usable.
