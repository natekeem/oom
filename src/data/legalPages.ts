    ...
    sections: [
      {
        heading: "수집하는 정보",
        paragraphs: [
          "오픽온미는 선택적으로 Google 로그인과 Supabase 계정 서비스를 제공합니다. 로그인 시 계정 식별자, 이메일, 표시 이름, 프로필 이미지와 가입 정보를 인증 및 계정 표시에 사용하며 Supabase Auth와 프로필 데이터베이스에 보관합니다. 로그인을 유지하기 위한 세션 정보는 현재 브라우저에 저장됩니다. 로그인 후 학습 설정, 명시적으로 완료한 학습 활동, 실전 연습의 문항 식별자와 완료·시간 정보가 저장될 수 있으며 녹음과 전사 원문은 이 학습 기록에 저장하지 않습니다. 계정 정보 삭제는 아래 문의 이메일로 요청할 수 있습니다. 사용자가 입력한 내부 LLM 설정은 현재 브라우저의 localStorage에만 저장되며, 오픽온미 서버로 전송되지 않습니다.",
          "OOM 관리형 AI 피드백을 요청하면 질문·학습 맥락·답변 텍스트가 Supabase Edge Function을 거쳐 외부 AI 제공자 Google Gemini로 전달되어 처리됩니다. Phase 3.1에서는 원문 답변을 OOM DB에 별도 저장하지 않으며, 생성된 피드백과 사용량·토큰·예상 비용·오류 코드 등의 운영 정보를 저장합니다. 생성된 피드백에는 답변 내용의 일부가 반영될 수 있습니다. 관리형 AI로 녹음 파일을 업로드하지 않습니다. 외부 제공자의 정보 처리에는 해당 제공자의 정책이 적용되며 민감한 개인정보나 회사 기밀을 입력하지 마세요.",
  ...
  terms: {
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: ["특정 등급, 합격, 성적 향상은 보장하지 않습니다.", "OOM 관리형 AI 피드백은 로그인, 운영 상태 및 서버 사용 한도에 따라 제공됩니다. AI 피드백은 공식 OPIc 점수·등급 판정이 아니며 오류가 있을 수 있습니다.", "관리형 피드백 요청 시 답변 텍스트가 외부 AI 제공자 Google Gemini에서 처리되며, OOM은 원문 답변을 별도 저장하지 않고 피드백 결과와 사용량 정보를 저장합니다. 관리형 AI로 녹음은 전송하지 않습니다.", "고급 사용자 설정의 STT·LLM은 사용자가 지정한 외부 endpoint를 브라우저에서 직접 호출합니다.", "공유 PC에서는 localStorage에 저장된 설정을 사용 후 삭제하는 것이 좋습니다."],
      },
      {
        heading: "콘텐츠와 책임",
