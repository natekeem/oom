...
export function AboutCourseSelector<TCourseId extends string>({
  ...
  return (
    ...
    >
      ...
      <h2 id="about-course-heading" className="mt-2 text-[17px] font-bold tracking-tight text-zinc-900 dark:text-white">무엇을 준비할지</h2>
      <p className="text-[10px] leading-[1.45] text-zinc-600 dark:text-[#858d9d]">반복해서 쓸 이야기의 맥락과 소재를 정합니다.</p>
          ...
          return (
            <button
              ...
              className={`relative z-10 flex w-full items-center justify-between rounded-lg border px-2 py-1.5 text-left transition duration-150 ${
                selected
                  ? "border-indigo-500 bg-indigo-100 text-indigo-900 dark:border-[#6964ee] dark:bg-[#1b1c2b] dark:text-white"
            ...
            >
              ...
              {course.helper && <span className="text-[8px] text-zinc-500 dark:text-[#777e8f]">{course.helper}</span>}
            </button>
          );
        })}
      </div>
