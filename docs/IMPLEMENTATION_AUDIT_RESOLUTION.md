...
| blueprint가 paragraph index에 묶임 | ANSWER, SCENE/ACTION, RESULT, optional EXPANSION 기능 블록으로 전환 | Resolved | `ScriptTrainingTabs.test.tsx` |
...
| Advanced 답변이 길고 filler가 필수처럼 보임 | 원문을 보존한 채 CORE/OPTIONAL EXPANSION으로 표시하고 filler를 선택 recovery phrase로 설명 | Resolved | `ScriptDetail.tsx`; visual QA |
