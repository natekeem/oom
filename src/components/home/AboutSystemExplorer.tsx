...
>({
  ...
  return (
    <section className="grid min-h-0 grid-cols-1 gap-3 lg:grid-cols-[270px_minmax(0,1fr)]">
      <div className="grid min-h-0 grid-cols-1 gap-2.5 sm:grid-cols-2 lg:grid-cols-1 lg:grid-rows-[minmax(0,1fr)_minmax(0,1fr)_auto]">
        ...
        <button
          type="button"
          className={`col-span-1 rounded-xl border p-2.5 text-[9px] transition duration-200 sm:col-span-2 lg:col-span-1 ${
      ...
      <div className="flex min-h-0 flex-col gap-3">
        ...
        <div className="flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="text-[9px] text-zinc-500 dark:text-[#8d94a3]">
            ...
            <div className="mt-1 text-[8px] text-zinc-400 dark:text-zinc-500">AI 피드백은 공식 OPIc 점수·등급 판정이 아닙니다.</div>
          </div>
          
          <div className="flex shrink-0 items-center gap-2">
            <Link to="/training/" className="inline-flex items-center justify-center rounded-lg bg-indigo-600 px-3 py-2 text-[10px] font-bold text-white no-underline transition hover:bg-indigo-700 dark:bg-[#6259f4] dark:hover:bg-[#5148e7]">
              실전 훈련 둘러보기
            </Link>
            <Link to="/exam-guide/" className="inline-flex items-center justify-center rounded-lg border border-zinc-300 bg-white px-3 py-2 text-[10px] font-bold text-zinc-700 no-underline transition hover:bg-zinc-50 dark:border-[#343743] dark:bg-transparent dark:text-[#d9dbe1] dark:hover:bg-[#1a1c23]">
              수험 가이드
            </Link>
          </div>
        </div>
