...
const categoryLabels = {
  ...
  "step5-roleplay-example": "STEP 5 level-specific answer examples",
  ...
  return courses.flatMap((course) =>
    levels.map((level) => {
      ...
      return {
        ...
        roleplayAnswerExamples: selected.filter(
          (item) => item.category === "step5-roleplay-example",
        ).length,
...
- Expanded static-first candidate inventory, including resolved STEP 4 variants, STEP 5 prompts, and STEP 5 answer examples: **${total.logicalItems} logical / ${total.uniqueTexts} globally unique texts**.
  ...
  try {
      ...
      for (const level of levels) {
        ...
        for (const roleplay of resolvedContext.roleplays) {
          records.push(
            ...
            record({
              logicalId: `roleplay:${course.id}:${level.id}:${roleplay.id}:answer`,
              sourceType: "roleplay-example",
              category: "step5-roleplay-example",
              staticClass: "static",
              currentTtsConsumer: true,
              courseId: course.id,
              levelId: level.id,
              roleplayId: roleplay.id,
              sourcePath: `src/data/training/courses/${course.id}/roleplays.ts`,
              sourceKey: `${roleplay.id}.levels.${level.id}.englishExample`,
    ...
    const audit = {
      ...
      summary: {
        ...
        step5RoleplayAnswerExamples: summarizeInventory(
          records.filter((item) => item.category === "step5-roleplay-example"),
        ),
