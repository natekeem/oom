...
실제 훈련은 사이드바의 **OPIc 실전 훈련하기** 아래 STEP 1~6으로 구성됩니다.

1. **목표 구간 · 코스 설정**: 1구간(AL), 2구간(IH/IM3), 3구간(IM2/IM1) 중 목표 구간과 학습 코스(Everyday & Getaway, Culture & City, Nature & Weekend)를 설정합니다.
2. **서베이 고정**: 실제형 설문 목록에서 선택한 코스 맞춤 추천 조합을 확인하고 연습 모드로 훈련합니다.
3. **난이도 설정**: 선택한 구간에 맞춘 권장 난이도(5-5, 4-4, 3-3)와 평가 초점을 확인합니다.
4. **만능 스크립트**: 코스별 4대 핵심 스토리(야외/여행, 실내/휴식, 운동/취미, 집/거주지)를 질문별 변형과 답변 설계도로 익힙니다.
5. **롤플레이 공식**: 6단계 만능 해결 공식(상황 → 문제 → 질문 → 대안 1 → 대안 2 → 마무리)과 필수 만능 표현을 익히고 코스별 실전 시나리오를 훈련합니다.
6. **실전 연습**: 목표 구간·코스 맞춤 질문, 60/90/120초 타이머, 녹음, 텍스트 답변, AI 피드백을 사용합니다.
