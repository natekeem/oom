  ...
  about: {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          "Course는 무슨 이야기를 준비할지 정하고, Level은 같은 이야기를 얼마나 깊게 말할지 정합니다.",
          "학습자는 익숙한 코스의 장면을 먼저 고른 뒤 목표 구간에 맞춰 핵심 설명, 행동과 이유, 변화와 관점까지 답변 밀도를 단계적으로 확장합니다.",
        ],
        bullets: ["Course: 서베이와 익숙한 장면의 맥락", "Level: 3구간에서 1구간까지 답변 밀도"],
      },
      {
        heading: "6 STEP 훈련 흐름",
        paragraphs: [
          "목표·코스 → 서베이 → 난이도 → 스크립트 → 롤플레이 → 실전 연습이 한 흐름으로 이어집니다.",
          "마지막에는 실제형 질문을 듣고 답한 뒤 내 녹음을 복기하고, 같은 질문에 다시 말해 수정한 한 가지를 바로 적용합니다.",
        ],
        links: [
    ...
    sections: [
      {
        heading: "수집하는 정보",
        paragraphs: [
          "오픽온미는 선택적으로 Google 로그인과 Supabase 계정 서비스를 제공합니다. 로그인 시 계정 식별자, 이메일, 표시 이름, 프로필 이미지와 가입 정보를 인증 및 계정 표시에 사용하며 Supabase Auth와 프로필 데이터베이스에 보관합니다. 로그인을 유지하기 위한 세션 정보는 현재 브라우저에 저장됩니다. 로그인 후 학습 설정, 명시적으로 완료한 학습 활동, 실전 연습의 문항 식별자와 완료·시간 정보가 저장될 수 있으며 녹음과 전사 원문은 이 학습 기록에 저장하지 않습니다. 계정 정보 삭제는 아래 문의 이메일로 요청할 수 있습니다. 사용자가 입력한 커스텀 LLM endpoint·model·options는 현재 브라우저의 localStorage에 저장되고, API key는 기본적으로 현재 탭의 sessionStorage에 저장됩니다. 사용자가 영구 저장을 명시한 경우에만 key가 localStorage에 남습니다. 커스텀 설정과 key는 오픽온미 서버로 전송되지 않습니다.",
          "OOM 관리형 AI를 요청하면 기능에 필요한 질문·학습 맥락·답변 또는 작성 입력이 Supabase Edge Function을 거쳐 외부 AI 제공자 Google Gemini로 전달되어 처리됩니다. 원문 입력은 OOM DB에 별도 저장하지 않으며, 검증된 피드백·생성 결과와 기능·버전·사용량·토큰·예상 비용·오류 코드 등의 운영 정보를 저장합니다. 생성 결과에는 입력 내용의 일부가 반영될 수 있습니다. 관리형 AI로 녹음 파일을 업로드하지 않습니다. 외부 제공자의 정보 처리에는 해당 제공자의 정책이 적용되며 민감한 개인정보나 회사 기밀을 입력하지 마세요. 커스텀 endpoint가 설정된 경우에는 브라우저가 해당 endpoint를 직접 호출하며 관리형 사용 기록을 만들지 않습니다.",
      ...
      },
      {
        ...
        paragraphs: [
          "AI 설정 화면에서 사용자가 입력하는 endpoint, 인증 방식, 모델 이름과 options는 현재 브라우저의 localStorage에 저장됩니다. API key는 기본적으로 현재 탭의 sessionStorage에 저장되고, 사용자가 영구 저장을 명시한 경우에만 localStorage에 저장됩니다. 이 값은 오픽온미 서버로 별도 저장되지 않지만, 같은 기기를 함께 쓰는 경우 저장한 설정이 남을 수 있습니다.",
  ...
  terms: {
    ...
    sections: [
      ...
      },
      {
        ...
        bullets: ["특정 등급, 합격, 성적 향상은 보장하지 않습니다.", "커스텀 LLM endpoint가 설정되어 있으면 모든 LLM 기능에서 브라우저가 해당 endpoint를 직접 호출합니다. 설정이 없을 때 관리형 AI가 사용되며 로그인, 운영 상태 및 기능별 서버 사용 한도에 따라 제공됩니다.", "AI 결과는 공식 OPIc 점수·등급 판정이 아니며 오류가 있을 수 있습니다. 관리형 요청 시 기능 입력이 외부 AI 제공자 Google Gemini에서 처리되며, OOM은 원문 입력을 별도 저장하지 않고 검증된 결과와 사용량 정보를 저장합니다. 관리형 AI로 녹음은 전송하지 않습니다.", "커스텀 STT·LLM 자격 증명은 사용자가 지정한 외부 endpoint로만 전달되며 관리형 서버로 보내지지 않습니다.", "공유 PC에서는 저장된 설정을 사용 후 삭제하고 API key 영구 저장을 피하는 것이 좋습니다."],
      },
      {
        heading: "콘텐츠와 책임",
