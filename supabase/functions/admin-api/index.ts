...
import { errorResponse, handleCorsPreflight } from "./cors.ts";
...
export async function handleAdminApiRequest(req: Request): Promise<Response> {
  ...
  if (req.method !== "GET") {
    return errorResponse(req, 405, "METHOD_NOT_ALLOWED", "지원하지 않는 HTTP 메서드입니다.");
  }
  ...
  try {
    // 4. Route dispatcher
    if (path === "/me") {
      return handleMe(req, adminUser);
    }
    if (path === "/overview") {
      return await handleOverview(req, adminClient);
    }
    if (path === "/users") {
      return await handleListUsers(req, adminClient);
    }
    if (path.startsWith("/users/")) {
      const userId = path.slice("/users/".length).trim();
      if (!userId) {
        return errorResponse(req, 400, "BAD_REQUEST", "사용자 ID가 제공되지 않았습니다.");
      }
      return await handleGetUserDetail(req, adminClient, userId);
    }
    if (path === "/learning") {
      return await handleLearning(req, adminClient);
    }
    if (path === "/audit") {
      return await handleAudit(req, adminClient, adminUser.role);
    }

    return errorResponse(req, 404, "NOT_FOUND", "요청하신 엔드포인트를 찾을 수 없습니다.");
  } catch (err) {
    console.error("[OOM Admin API Uncaught Error]", err);
    return errorResponse(req, 500, "SERVER_ERROR", "서버 내부 오류가 발생했습니다.");
  }
}
