...
// @ts-nocheck
/**
 * GPU fluid cursor adapted for OOM from Pavel Dobryakov's WebGL Fluid Simulation.
 * Copyright (c) 2017 Pavel Dobryakov. Licensed under the MIT License.
 * See THIRD_PARTY_NOTICES.md.
 */

export function initAgencyFluidCursor(
  ...
  let config = {
    SIM_RESOLUTION: quality === "high" ? 128 : 96,
    DYE_RESOLUTION: quality === "high" ? 1440 : 768,
...
`,
  ...
  const handleFirstMouseMove = (event) => {
    const pointer = pointers[0];
    const posX = scaleByPixelRatio(event.clientX);
    const posY = scaleByPixelRatio(event.clientY);
