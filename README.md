...
루트 `/`는 OOM의 학습 방식을 설명하는 독립형 브랜드 랜딩입니다. AppShell과 Sidebar를 사용하지 않으며, Hero의 O signal을 이루던 입자가 화면 밖으로 흩어진 뒤 중간 section 동안 바깥에 머물고 Final에서 다시 O로 모입니다. 중간 section의 얇은 signal trace는 별도 DOM/CSS 장식으로 유지하며, 동일한 R3F particle pool은 O의 이탈과 재결집만 담당합니다. STEP 6 미리보기는 실제 `ExamScreenShell`을 presentation-only로 재사용하고 Recorder·STT·AI·훈련 상태를 mount하지 않습니다. 유체 커서와 시그니처 오브젝트 WebGL, scroll motion은 progressive enhancement이고, mobile·low capability·`prefers-reduced-motion`에서는 동일한 카피와 CTA를 가진 정적 흐름으로 전환됩니다.
...
### Static TTS asset 재생성

FFmpeg와 ffprobe가 `PATH`에 있거나 `OOM_FFMPEG_PATH`/`OOM_FFPROBE_PATH`로 지정된 개발 환경에서 아래 명령을 실행합니다. 브라우저 generator가 중단되어도 이미 검증된 exact target은 다시 만들지 않습니다.

```bash
npm run tts:generate -- --pilot
