...
`src/lib/stt.ts` calls the user-configured STT endpoint directly. Every LLM feature calls `src/features/ai/runAiFeature.ts`: a usable custom LLM endpoint is called directly from the browser; otherwise the managed `ai-api` path is used. A malformed non-empty custom configuration is an error, not permission to send the input elsewhere. Endpoint/model/options are stored in localStorage; API keys use sessionStorage by default, with explicit remember-key opt-in for persistent storage. Endpoint CORS support is required. No custom key, transcript or recording is sent to Supabase.
...
LLM routing is automatic: usable custom endpoint → direct browser call; no custom endpoint → ai-api → verified auth → FREE entitlement → per-feature atomic quota reservation → Gemini → validated result + usage persistence. There is no cross-provider fallback. No managed STT, audio storage, billing or paid subscription entitlement is implemented. See [setup](SUPABASE_SETUP.md).
